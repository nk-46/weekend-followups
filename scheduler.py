...
scheduler.add_job(
    ...
    trigger=CronTrigger(day_of_week="sat", hour=19, minute=0, misfire_grace_time=300, timezone=IST),
...
scheduler.add_job(
    ...
    trigger=CronTrigger(day_of_week="mon", hour=5, minute=0, misfire_grace_time=300, timezone=IST),
...
scheduler.add_job(
    ...
    trigger=CronTrigger(day=1, hour=0, minute=0, misfire_grace_time=300, timezone=IST)
)
        ...
        if not job.next_run_time:
            ...
            if job.id == "set_true_job":
                scheduler.add_job(
                    main,
                    id="set_true_job",
                    trigger=CronTrigger(day_of_week="sat", hour=5, minute=0, misfire_grace_time=300, timezone=IST),
                    kwargs={"action": "set_true"}
                )
